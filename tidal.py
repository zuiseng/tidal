...
class TidalClient(Client):
    ...
    def resolve_url(self, url: str) -> Tuple[MediaType, Optional[Media]]:
        ...
        if last_index == -1:
            raise ValueError(
                'Couldn\'t parse that URL. Try again!'
            )
        ...
        if not re.match(r"^\d+$", media_id):
            raise ValueError(
                'That URL doesn\'t end in an ID. Try again!'
            )
    ...
    def _decrypt_security_token(security_token: bytes) -> Tuple[bytes, bytes]:
        # from https://github.com/yaronzz/Tidal-Media-Downloader/blob/master/TIDALDL-PY/tidal_dl/decryption.py,
